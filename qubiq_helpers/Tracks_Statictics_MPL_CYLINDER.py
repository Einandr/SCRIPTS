...
data_type = DataType.FLUENT


if data_type == DataType.QUBIQ:
    path = r'D:\YASIM\VORON\2025_09_MPL\TASK_DECEMBER\04_TT2_OLD_MODELS\01_3comp_basic'
elif data_type == DataType.FLUENT:
...
if data_type == DataType.QUBIQ:
    df = pd.read_csv(f'{path}/{dir_out}/tracks_all.txt', dtype=float)
elif data_type == DataType.FLUENT:
    df = pd.read_csv(f'{path}/{dir_out}/points_for_tecplot_FLUENT.csv', dtype=float)
else:
    raise ValueError("Неизвестный тип данных")
...
if data_type == DataType.FLUENT:
    ...
    # Вычисляем разницу диаметра и времени для каждого трека
    df['diameter_diff'] = df.groupby('track_id')['diameter'].diff().bfill()
    df['time_diff'] = df.groupby('track_id')['time'].diff().bfill()
    # Вычисляем скорость уменьшения радиуса (линейную скорость горения)
    df['burning_rate'] = -df['diameter_diff'].div(df['time_diff'], fill_value=0)/1000/2
    # Удаляем вспомогательные столбцы, если они больше не нужны
    # df.drop(columns=['diameter_diff', 'time_diff'], inplace=True)
...
plot_all_tracks(grouped, ['burning_rate'], ['-'], [r'$линейная\ скорость\ горения\ [мм/с]$'], r'$u\ (мм/c)$', 'burning_rate_x',
                x_column='CoordinateX',
                x_label=r'$X\ (м)$',
                show_track_legend=False,
                max_legend_entries=5,

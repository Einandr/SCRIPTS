...
path_verification = r'D:\YASIM\VORON\2026_12_Iskra_OPZ\Validation\qubiq_1st_more'
...
if QUBIQ:
    if PARTICLES:
        ...
        # validation 1st
        path_qubiq_particles_01 = r'D:\YASIM\VORON\2026_12_Iskra_OPZ\QUBIQ\validation_from_oleg_1st\part\tracks\averaged_data_by_time.csv'
    ...
    if GAS:
        ...
        # validation 1st
        path_qubiq_gas_01 = r'D:\YASIM\VORON\2026_12_Iskra_OPZ\QUBIQ\validation_from_oleg_1st\run\slices\combined_results.csv'
...
y_limits = {
    ...
    'gas_temperature': [0, 2500],
    'gas_pressure': [0, 500000],
    'gas_velocity': [0, 1250],
    'gas_Y_air': [0, 1.1],
    'gas_Y_cp': [0, 1.1],
    'gas_Y_gpg': [0, 0.1],
    'gas_mass_flow_rate': [0, 3]
}
...
# iskra OPZ validation
mfr_fuel_particles_in = 0.181225
mfr_fuel_gas_in = 0.148275
mfr_oxidizer_in = 2.4992
...
if QUBIQ and PARTICLES:
    ...
    # При условии альфа > 1:
    # ГРУБАЯ ОЦЕНКА ЗА НЕИМЕНИЕМ ДАННЫХ О КОМПОНЕНТАХ СКОРОСТИ И ПЛОТНОСТИ
    diameter_initial = 5
    density = 2584
    df_qubiq_particles_01['mass'] = density * math.pi * pow(df_qubiq_particles_01['diameter'], 3) / 6 / 1e18
    df_qubiq_particles_01['Combustion_Efficiency'] = np.maximum(1 - (pow(df_qubiq_particles_01['diameter'], 3) / pow(diameter_initial, 3)), 0)
...
if QUBIQ and PARTICLES:
    plot_result(f"{pic_base_name}particle_diameter", x_t_label, r'$d,\ мкм$', (df_qubiq_particles_01, ['diameter'], [style_Q], ['СПК «ГиперКуб-ГРАФ»'], y_limits['particle_diameter'], colors),
                GOST=GOST, x_limits=(x_limit_low, x_limit_high), swap_axes=False, line_width=line_width)
    plot_result(f"{pic_base_name}particle_mass", x_t_label, r'$m,\ кг$', (df_qubiq_particles_01, ['mass'], [style_Q], ['СПК «ГиперКуб-ГРАФ»'], y_limits['particle_mass'], colors),
                GOST=GOST, x_limits=(x_limit_low, x_limit_high), swap_axes=False, line_width=line_width)
    plot_result(f"{pic_base_name}particle_combustion_efficiency", x_t_label, r'$\eta$', (df_qubiq_particles_01, ['Combustion_Efficiency'], [style_Q], ['СПК «ГиперКуб-ГРАФ»'], y_limits['combustion_efficiency'], colors),
...
if QUBIQ and GAS:
    ...
    plot_result(f"{pic_base_name}gas_etta_QUBIQ_03", x_t_label, r'$\eta$', (df_qubiq_gas_01, ['etta_Y_oxidizer'], [style_Q], ['СПК «ГиперКуб-ГРАФ» - полнота по массовой доле окислителя'], y_limits['combustion_efficiency'], colors), GOST=GOST, x_limits=(x_limit_low, x_limit_high), swap_axes=False, line_width=line_width, horizontal_lines=[1])

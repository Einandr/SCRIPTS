...
path_verification = r'D:\YASIM\VORON\2026_15_Spalding_Combustion\verification'
...
if verify_FLUENT:
    df_fluent_particles_01 = pd.read_csv(path_fluent_particles_01, index_col='time_local')

if verify_gas:
    df_fluent_gas_01 = pd.read_csv(path_fluent_gas_01, index_col='CoordinateX')

if verify_QUBIQ:
    df_qubiq_particles_01 = pd.read_csv(path_qubiq_particles_01, index_col='time_local')
...
if verify_QUBIQ:
    df_qubiq_particles_01 = df_qubiq_particles_01.rename(columns={
        'diameter': 'Diameter',
        'density': 'Density',
        'mass': 'Mass',
...
if verify_FLUENT:
    df_fluent_particles_01 = df_fluent_particles_01.rename(columns={
        'diameter': 'Diameter',
        'density': 'Density',
        'mass': 'Mass',
        'nparticles': 'Nparticles',
...
if verify_QUBIQ and not verify_FLUENT:
    plot_result(f"{pic_base_name}particle_diameter", x_t_label_particles, r'$d,\ мкм$',
                (df_qubiq_particles_01, ['Diameter'], [style_Q], ['СПК «ГиперКуб-ГРАФ»'], y_limits['particle_diameter'], colors),
                GOST=GOST, x_limits=(t_limit_low, t_limit_high), swap_axes=False, line_width=line_width)
    plot_result(f"{pic_base_name}particle_temperature", x_t_label_particles, r'$T,\ K$',
                (df_qubiq_particles_01, ['Temperature'], [style_Q], ['СПК «ГиперКуб-ГРАФ»'], y_limits['particle_temperature'], colors),
                GOST=GOST, x_limits=(t_limit_low, t_limit_high), swap_axes=False, line_width=line_width)
    plot_result(f"{pic_base_name}particle_mass", x_t_label_particles, r'$m,\ кг$',
                (df_qubiq_particles_01, ['Mass'], [style_Q], ['СПК «ГиперКуб-ГРАФ»'], y_limits['particle_mass'], colors),
                GOST=GOST, x_limits=(t_limit_low, t_limit_high), swap_axes=False, line_width=line_width)
    plot_result(f"{pic_base_name}particle_density", x_t_label_particles, r'$\rho\ \left(\frac{кг}{м^3}\right)$',
                (df_qubiq_particles_01, ['Density'], [style_Q], ['СПК «ГиперКуб-ГРАФ»'], y_limits['particle_density'], colors),
                GOST=GOST, x_limits=(t_limit_low, t_limit_high), swap_axes=False, line_width=line_width)
...
if verify_QUBIQ and verify_FLUENT:
    plot_result(f"{pic_base_name}particle_diameter", x_t_label_particles, r'$d,\ мкм$',
                (df_qubiq_particles_01, ['Diameter'], [style_Q], ['СПК «ГиперКуб-ГРАФ»'], y_limits['particle_diameter'], colors),
                (df_fluent_particles_01, ['Diameter'], [style_F], ['ПК «ANSYS Fluent»'], y_limits_default, colors),
                GOST=GOST, x_limits=(t_limit_low, t_limit_high), swap_axes=False, line_width=line_width)
    plot_result(f"{pic_base_name}particle_temperature", x_t_label_particles, r'$T,\ K$',
                (df_qubiq_particles_01, ['Temperature'], [style_Q], ['СПК «ГиперКуб-ГРАФ»'], y_limits['particle_temperature'], colors),
                (df_fluent_particles_01, ['Temperature'], [style_F], ['ПК «ANSYS Fluent»'], y_limits_default, colors),
                GOST=GOST, x_limits=(t_limit_low, t_limit_high), swap_axes=False, line_width=line_width)
    ...
    plot_result(f"{pic_base_name}particle_mass", x_t_label_particles, r'$m,\ кг$',
                (df_qubiq_particles_01, ['Mass'], [style_Q], ['СПК «ГиперКуб-ГРАФ»'], y_limits['particle_mass'], colors),
                (df_fluent_particles_01, ['Mass'], [style_F], ['ПК «ANSYS Fluent»'], y_limits_default, colors),
                GOST=GOST, x_limits=(t_limit_low, t_limit_high), swap_axes=False, line_width=line_width)
    plot_result(f"{pic_base_name}particle_density", x_t_label_particles, r'$\rho\ \left(\frac{кг}{м^3}\right)$',
                (df_qubiq_particles_01, ['Density'], [style_Q], ['СПК «ГиперКуб-ГРАФ»'], y_limits['particle_density'], colors),
                (df_fluent_particles_01, ['Density'], [style_F], ['ПК «ANSYS Fluent»'], y_limits_default, colors),
                GOST=GOST, x_limits=(t_limit_low, t_limit_high), swap_axes=False, line_width=line_width)
...
# ГАЗ
if verify_gas:
    plot_result(f"{pic_base_name}gas_temperature", x_t_label_gas, r'$T,\ K$', (df_qubiq_gas_01, ['Temperature'], [style_Q], ['СПК «ГиперКуб-ГРАФ»'], y_limits['gas_temperature'], colors),
    ...
    plot_result(f"{pic_base_name}gas_pressure", x_t_label_gas, r'$p,\ Па$', (df_qubiq_gas_01, ['Pressure'], [style_Q], ['СПК «ГиперКуб-ГРАФ»'], y_limits['gas_pressure'], colors),
    ...
    plot_result(f"{pic_base_name}gas_velocity", x_t_label_gas, r'$\upsilon,\ м/с$', (df_qubiq_gas_01, ['Velocity'], [style_Q], ['СПК «ГиперКуб-ГРАФ»'], y_limits['gas_velocity'], colors),
    ...
    plot_result(f"{pic_base_name}gas_Y", x_t_label_gas, r'$Y$', (df_qubiq_gas_01, ['Y_air', 'Y_cp', 'Y_gpg'], [style_Q, style_Q, style_Q], ['СПК «ГиперКуб-ГРАФ» - массовая доля воздуха', 'СПК «ГиперКуб-ГРАФ» - массовая доля ПС', 'СПК «ГиперКуб-ГРАФ» - массовая доля ГПГ'], y_limits['gas_Y_air'], colors),
    ...
    plot_result(f"{pic_base_name}gas_Y2", x_t_label_gas, r'$Y$', (df_qubiq_gas_01, ['Y_air', 'Y_cp'], [style_Q, style_Q], ['СПК «ГиперКуб-ГРАФ» - массовая доля воздуха', 'СПК «ГиперКуб-ГРАФ» - массовая доля ПС'], y_limits['gas_Y_air'], colors),
    ...
    plot_result(f"{pic_base_name}gas_Y3", x_t_label_gas, r'$Y$', (df_qubiq_gas_01, ['Y_gpg'], [style_Q], ['СПК «ГиперКуб-ГРАФ» - массовая доля ГПГ'], y_limits['gas_Y_gpg'], colors),
    ...
    plot_result(f"{pic_base_name}gas_etta_01", x_t_label_gas, r'$\eta$',
    ...
    plot_result(f"{pic_base_name}gas_etta_02", x_t_label_gas, r'$\eta$',
                (df_qubiq_gas_01, ['etta_mfr_products', 'etta_mfr_oxidizer', 'etta_Y_products', 'etta_Y_oxidizer'], [style_Q] * 4, ['СПК «ГиперКуб-ГРАФ» - полнота по расходу продуктов', 'СПК «ГиперКуб-ГРАФ» - полнота по расходу окислителя', 'СПК «ГиперКуб-ГРАФ» - полнота по массовой доле продуктов', 'СПК «ГиперКуб-ГРАФ» - полнота по массовой доле окислителя'], y_limits['combustion_efficiency'], colors),
    ...
    plot_result(f"{pic_base_name}gas_etta_MFR", x_t_label_gas, r'$\eta$',
    ...
    plot_result(f"{pic_base_name}gas_etta_Y", x_t_label_gas, r'$\eta$',

...
file_props_excel = 'Свойства_н-гептан.xlsx'
out_FLUENT = True
out_QUBIQ = False
name = 'n-heptane-liquid'
liquid = True
...
# тут будем хранить датафреймы с единичным столбцом свойств
...
if calculate_diffusion:
    # n-Heptane data and formula
    g_M_vapor = 100.203     # vapor molar mass [kg/mol]
    g_M = 28.288239         # gas molar mass [kg/mol]
    g_p_reference = 500000  # reference pressure [Pa]
    g_etta_vapor = 147.18   # vapor molecular volume [sm^3/mol]
    g_etta = 20.1           # gas molecular volume [sm^3/mol]

    def D_n_heptane_air(T, P, M_v, M, etta_v, etta):
        return 0.01 * pow(T, 1.75) / P * pow((M_v + M) / (M_v * M), 1/2) / pow((pow(etta_v, 1/3)+pow(etta, 1/3)), 2)
    ...
    for T in temperatures:
        df.loc[T, diffusion_column_name] = D_n_heptane_air(T, g_p_reference, g_M_vapor, g_M, g_etta_vapor, g_etta)

...
class Component:
    def __init__(self, path_yaml_gas, path_yaml_liquid, name, formula):
        if not os.path.exists(path_yaml_gas):
            raise FileNotFoundError(f"Файл {path_yaml_gas} не найден!")
        try:
            with open(path_yaml_gas, 'r', encoding='utf-8') as f:
                self.data_gas = yaml.safe_load(f)
        except yaml.YAMLError as e:
            raise yaml.YAMLError(f"Ошибка при чтении YAML-файла {path_yaml_gas}: {str(e)}")

        if not os.path.exists(path_yaml_liquid):
            raise FileNotFoundError(f"Файл {path_yaml_liquid} не найден!")
        try:
            with open(path_yaml_liquid, 'r', encoding='utf-8') as f:
                self.data_liquid = yaml.safe_load(f)
        except yaml.YAMLError as e:
            raise yaml.YAMLError(f"Ошибка при чтении YAML-файла {path_yaml_liquid}: {str(e)}")

        if len(self.data_gas['formation_heat']) != 1:
            raise ValueError("Ожидался единственный ключ в gas_formation_heat!")
        if len(self.data_liquid['formation_heat']) != 1:
            raise ValueError("Ожидался единственный ключ в liquid_formation_heat!")
        if next(iter(self.data_gas['formation_heat'].keys())) != next(iter(self.data_liquid['formation_heat'].keys())):
            raise ValueError("Для gas_formation_heat и liquid_formation_heat должны быть одинаковые значения референсной температуры!")

        self.name = name
        self.formula = formula
        self.temperature_boiling = 400

        self.temperature_reference = next(iter(self.data_gas['formation_heat'].keys()))
        self.gas_formation_heat = self.data_gas['formation_heat'][self.temperature_reference]
        self.liquid_formation_heat = self.data_liquid['formation_heat'][self.temperature_reference]

        self.gas_heat_capacity = self.data_gas['heat_capacity']
        self.liquid_heat_capacity = self.data_liquid['heat_capacity']

        print(self.gas_formation_heat)
        print(self.liquid_formation_heat)

        # gas enthalpy
        print('\nCalculating gas enthalpy for species', self.formula, '...')
        self.__x_values = list(self.gas_heat_capacity.keys())
        self.__y_values = []
        for item in self.__x_values:
            self.__y_values.append(integrate_value(item, self.gas_heat_capacity))
        self.gas_enthalpy = dict(zip(self.__x_values, self.__y_values))

        # liquid enthalpy
        print('\nCalculating liquid enthalpy for species', self.formula, '...')
        self.__x_values = list(self.liquid_heat_capacity.keys())
        self.__y_values = []
        for item in self.__x_values:
            self.__y_values.append(integrate_value(item, self.liquid_heat_capacity))
        self.liquid_enthalpy = dict(zip(self.__x_values, self.__y_values))
        # print(self.liquid_enthalpy)

        print(f"Gas enthalpy for {self.formula}: {self.gas_enthalpy}")
        print(f"Liquid enthalpy for {self.formula}: {self.liquid_enthalpy}")

    def get_gas_heat_capacity(self, temperature):
        return piecewise_value(temperature, self.gas_heat_capacity)

    def get_liquid_heat_capacity(self, temperature):
        return piecewise_value(temperature, self.liquid_heat_capacity)

    def get_gas_enthalpy(self, temperature):
        return piecewise_value(temperature, self.gas_enthalpy)

    def get_liquid_enthalpy(self, temperature):
        return piecewise_value(temperature, self.liquid_enthalpy)

    def get_gas_full_enthalpy(self, temperature):
        print(self.temperature_reference, self.gas_formation_heat)
        return self.gas_formation_heat + self.get_gas_enthalpy(temperature) - self.get_gas_enthalpy(self.temperature_reference)

    def get_liquid_full_enthalpy(self, temperature):
        print(self.temperature_reference, self.liquid_formation_heat)
        return self.liquid_formation_heat + self.get_liquid_enthalpy(temperature) - self.get_liquid_enthalpy(self.temperature_reference)
